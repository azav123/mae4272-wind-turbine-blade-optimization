...
- Max RPM: 2000
- Max stress: 29.33 MPa (44 MPa / 1.5 safety factor)
"""
...
MAX_RADIUS = HUB_RADIUS + MAX_BLADE_LENGTH  # m
MAX_RPM = 2000
...
def create_blade_geometry(pitch_angles, chord_distribution='polynomial'):
    """
    Create blade geometry arrays.
    ...
    r_list = np.linspace(HUB_RADIUS, MAX_RADIUS, N_RADIAL_POINTS)
    ...
    if chord_distribution == 'polynomial':
        ...
        c_root = 0.0508  # 2 inches in meters at hub
        c_tip = 0.0254   # 1 inch in meters at tip (half of c_root)
        n = 2  # quadratic exponent
        
        # Normalized distance from tip (1 at hub, 0 at tip)
        r_normalized = (MAX_RADIUS - r_list) / (MAX_RADIUS - HUB_RADIUS)
...
def calculate_power_at_wind_speed(U, pitch_angles, rpm, cl_interp, cd_interp,
                                   I_y_non_dim, I_z_non_dim, c_max_non_dim):
    """
    ...
    constraint_violation : bool
        True if stress or RPM constraints violated
    """
    r_list, theta_list, chord_list = create_blade_geometry(pitch_angles)
...
def objective_function(x, cl_interp, cd_interp, I_y_non_dim, I_z_non_dim, c_max_non_dim, verbose=False):
    """
    Objective function for optimization: negative expected power.
    
    Parameters:
    -----------
    x : array-like
        Optimization variables: [pitch_0, pitch_1, ..., pitch_19, rpm_target]
        First N_RADIAL_POINTS elements are pitch angles (degrees)
        Last element is target RPM
    
    Returns:
    --------
    neg_expected_power : float
        Negative of expected power (for minimization)
    """
    # Extract variables
    ...
    for U in U_samples:
        ...
        P, violation = calculate_power_at_wind_speed(
            U, pitch_angles, rpm_target, cl_interp, cd_interp, 
            I_y_non_dim, I_z_non_dim, c_max_non_dim)
        ...
        expected_power += P * pdf_val * (U_samples[1] - U_samples[0])
    ...
    if verbose:
        print(f"Pitch: {pitch_angles[0]:.1f}° to {pitch_angles[-1]:.1f}°, "
              f"RPM: {rpm_target:.0f}, Power: {expected_power:.3f} W, "
              f"Penalty: {penalty:.0e}")
    
    return -expected_power + penalty
...
def optimize_blade_design(cl_interp, cd_interp, I_y_non_dim, I_z_non_dim, c_max_non_dim,
    ...
    # Pitch angle bounds (degrees) - allow full range for flexibility
    for i in range(N_RADIAL_POINTS):
        bounds.append((-10.0, 45.0))
    
    # RPM bound
    bounds.append((100.0, MAX_RPM))
    
    print("Starting optimization...")
    print(f"Number of variables: {len(bounds)}")
...
def analyze_optimal_design(optimal_params, cl_interp, cd_interp, I_y_non_dim, I_z_non_dim, c_max_non_dim):
    ...
    for U in U_test:
        P, violation = calculate_power_at_wind_speed(
            U, pitch_angles, rpm_optimal, cl_interp, cd_interp, 
            I_y_non_dim, I_z_non_dim, c_max_non_dim)
...
def plot_optimal_design(optimal_params, cl_interp, cd_interp, I_y_non_dim, I_z_non_dim, c_max_non_dim):
    ...
    plt.tight_layout()
    plt.savefig('output/viterna_corrigan/optimal_blade_design.png', dpi=300)
    print("\nPlots saved to 'output/viterna_corrigan/optimal_blade_design.png'")
...
def main():
    ...
    cl_interp, cd_interp = load_polar_data()
    ...
    if result.success:
        ...
        df_output.to_csv('output/viterna_corrigan/optimal_blade_parameters.csv', index=False)
        print("\nOptimal parameters saved to 'output/viterna_corrigan/optimal_blade_parameters.csv'")
        
    else:
        print("\nOptimization did not converge. Try adjusting parameters.")
