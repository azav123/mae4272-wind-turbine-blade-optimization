...
1. Torque vs RPM analysis (900-2000 RPM and 900-3000 RPM ranges)
2. Power vs RPM analysis (900-2000 RPM and 900-3000 RPM ranges)
...
# Output directory
OUTPUT2_DIR = str(BASE_DIR / 'outputs' / 'viterna_corrigan')
...
df_smooth = pd.read_csv(str(RESULTS_DIR / 'smoothed_blade_parameters.csv'))
...
FAILURE_STRESS = 55e6  # Pa, ultimate stress
SAFETY_FACTOR = 1.5
MAX_STRESS = FAILURE_STRESS / SAFETY_FACTOR  # 36.67 MPa
WEIBULL_K = 5.0
WEIBULL_C = 5.0
N_RADIAL_POINTS = 10
N_SEGMENTS = N_RADIAL_POINTS - 1
B = 1  # Blades per analysis
START_RPM = 900  # Starting RPM for valid torque/power data
...
for rpm in rpm_range_2000:
    ...
    # Scale for 3-blade turbine and convert units
    torque_2000.append(Q * 300)  # N·m -> N·cm × 3 blades
    power_2000.append(P * 3)     # W × 3 blades
...
for rpm in rpm_range_3000:
    ...
    # Scale for 3-blade turbine and convert units
    torque_3000.append(Q * 300)  # N·m -> N·cm × 3 blades
    power_3000.append(P * 3)     # W × 3 blades
    stress_3000.append(max_sigma / 1e6)  # Convert to MPa

# Slice torque and power arrays to start from START_RPM
idx_plot_2000 = np.where(rpm_range_2000 >= START_RPM)[0][0]
rpm_plot_2000 = rpm_range_2000[idx_plot_2000:]
torque_plot_2000 = np.array(torque_2000)[idx_plot_2000:]
power_plot_2000 = np.array(power_2000)[idx_plot_2000:]

idx_plot_3000 = np.where(rpm_range_3000 >= START_RPM)[0][0]
rpm_plot_3000 = rpm_range_3000[idx_plot_3000:]
torque_plot_3000 = np.array(torque_3000)[idx_plot_3000:]
power_plot_3000 = np.array(power_3000)[idx_plot_3000:]

# Find optimal values (maximum power point)
max_power_idx = np.argmax(power_2000)
optimal_rpm = rpm_range_2000[max_power_idx]
optimal_torque_1blade = torque_2000[max_power_idx] / 300  # Convert back to 1-blade for calculations
optimal_torque_3blades = torque_2000[max_power_idx]       # Already scaled for 3 blades
optimal_power_3blades = power_2000[max_power_idx]         # Already scaled for 3 blades
optimal_max_stress = stress_2000[max_power_idx]

# For file output and calculations, keep 1-blade values
optimal_torque = optimal_torque_1blade
optimal_power = optimal_power_3blades / 3

print(f"\nOptimal Operating Point:")
print(f"  RPM: {optimal_rpm:.1f}")
print(f"  Torque: {optimal_torque_3blades:.2f} N·cm [3 blades]")
print(f"  Power: {optimal_power_3blades:.2f} W [3 blades]")
print(f"  Max Bending Stress: {optimal_max_stress:.3f} MPa")

# ============================================================================
# 1. TORQUE vs RPM (900-2000 RPM)
# ============================================================================

fig, ax = plt.subplots(1, 1, figsize=(10, 6))
ax.plot(rpm_plot_2000, torque_plot_2000, 'b-', linewidth=2)
if optimal_rpm >= START_RPM:
    ax.axvline(optimal_rpm, color='r', linestyle='--', linewidth=2,
               label=f'Optimal RPM ({optimal_rpm:.0f})')
ax.axhline(optimal_torque_3blades, color='g', linestyle='--', linewidth=2,
           label=f'Optimal Torque ({optimal_torque_3blades:.2f} N·cm)')
ax.set_xlabel('RPM')
ax.set_ylabel('Torque (N·cm) [3 blades]')
ax.set_title('Torque vs RPM (900-2000 RPM Range) [3 blades]')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT2_DIR, 'torque_vs_rpm_900_2000.png'), dpi=300)
print(f"\nSaved: {os.path.join(OUTPUT2_DIR, 'torque_vs_rpm_900_2000.png')}")

# ============================================================================
# 2. TORQUE vs RPM (900-3000 RPM)
# ============================================================================

fig, ax = plt.subplots(1, 1, figsize=(10, 6))
ax.plot(rpm_plot_3000, torque_plot_3000, 'b-', linewidth=2)
if optimal_rpm >= START_RPM:
    ax.axvline(optimal_rpm, color='r', linestyle='--', linewidth=2,
               label=f'Optimal RPM ({optimal_rpm:.0f})')
ax.axhline(optimal_torque_3blades, color='g', linestyle='--', linewidth=2,
...
ax.set_xlabel('RPM')
ax.set_ylabel('Torque (N·cm) [3 blades]')
ax.set_title('Torque vs RPM (900-3000 RPM Range) [3 blades]')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT2_DIR, 'torque_vs_rpm_900_3000.png'), dpi=300)
print(f"Saved: {os.path.join(OUTPUT2_DIR, 'torque_vs_rpm_900_3000.png')}")

# ============================================================================
# 3. POWER vs RPM (900-2000 RPM)
# ============================================================================

fig, ax = plt.subplots(1, 1, figsize=(10, 6))
ax.plot(rpm_plot_2000, power_plot_2000, 'g-', linewidth=2)
if optimal_rpm >= START_RPM:
    ax.axvline(optimal_rpm, color='r', linestyle='--', linewidth=2,
               label=f'Optimal RPM ({optimal_rpm:.0f})')
ax.axhline(optimal_power_3blades, color='purple', linestyle='--', linewidth=2,
           label=f'Max Power ({optimal_power_3blades:.2f} W)')
ax.set_xlabel('RPM')
ax.set_ylabel('Power Output (W) [3 blades]')
ax.set_title('Power vs RPM (900-2000 RPM Range) [3 blades]')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT2_DIR, 'power_vs_rpm_900_2000.png'), dpi=300)
print(f"Saved: {os.path.join(OUTPUT2_DIR, 'power_vs_rpm_900_2000.png')}")

# ============================================================================
# 4. POWER vs RPM (900-3000 RPM)
# ============================================================================

fig, ax = plt.subplots(1, 1, figsize=(10, 6))
ax.plot(rpm_plot_3000, power_plot_3000, 'g-', linewidth=2)
if optimal_rpm >= START_RPM:
    ax.axvline(optimal_rpm, color='r', linestyle='--', linewidth=2,
               label=f'Optimal RPM ({optimal_rpm:.0f})')
ax.axhline(optimal_power_3blades, color='purple', linestyle='--', linewidth=2,
...
ax.set_xlabel('RPM')
ax.set_ylabel('Power Output (W) [3 blades]')
ax.set_title('Power vs RPM (900-3000 RPM Range) [3 blades]')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT2_DIR, 'power_vs_rpm_900_3000.png'), dpi=300)
print(f"Saved: {os.path.join(OUTPUT2_DIR, 'power_vs_rpm_900_3000.png')}")
...
with open(os.path.join(OUTPUT2_DIR, 'final_report_summary.txt'), 'w') as f:
    ...
    f.write(f"Torque/Power RPM range: 900-3000 RPM\n")
    ...
    f.write(f"Ideal torque: {optimal_torque_3blades:.2f} N·cm [3 blades]\n")
    f.write(f"Ideal power: {optimal_power_3blades:.2f} W [3 blades]\n")
    ...
    f.write("-" * 30 + "\n")
    f.write("torque_vs_rpm_900_2000.png\n")
    f.write("torque_vs_rpm_900_3000.png\n")
    f.write("power_vs_rpm_900_2000.png\n")
    f.write("power_vs_rpm_900_3000.png\n")
...
print("Generated:")
print("  ✓ torque_vs_rpm_900_2000.png")
print("  ✓ torque_vs_rpm_900_3000.png")
print("  ✓ power_vs_rpm_900_2000.png")
print("  ✓ power_vs_rpm_900_3000.png")
...
print(f"  Ideal torque: {optimal_torque*300:.2f} N·cm [3 blades]")
print(f"  Ideal power: {optimal_power*3:.2f} W [3 blades]")
print(f"  Max bending stress: {optimal_max_stress:.3f} MPa")
